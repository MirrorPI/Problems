...
class Solution {
    ...
    public boolean canFit(Shape block, Shape blank) {
        ...
        for(int i=0; i<4; i++) {
            ...
            rotatedBlock = nomalize(rotate90(rotatedBlock));
        }
        
        return false;
    }
    public Shape nomalize(Shape shape) {
        Shape nomalizedShape = new Shape();
        int minX = 55;
        int minY = 55;
        ...
        for(Point p : shape.points) {
            nomalizedShape.add(new Point(p.x - minX, p.y - minY));
        }
        return nomalizedShape;
    }
    ...
    public void saveShape(List<Shape> shapes, int[][] map, int target) {
        ...
        for(int x=0; x<xLen; x++) {
            for(int y=0; y<yLen; y++) {
                ...
                shapes.add(findShapeByBFS(x, y, map, visited, target));
            }
        }
    }
